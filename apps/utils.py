...
class StdImageField(ImageField):
    ...
    def _set_thumbnail(self, instance=None, **kwargs):
        if getattr(instance, self.name):
            for i, size in enumerate(self.sizes):
                k = i + 1
                setattr(getattr(instance, self.name), 'image%d' % k, '%s%s-%s.jpg' % (self.upload_to, instance._get_pk_val(), k))
...
def petsnet(request):
	return {
        ...
        'announcements_types': ("sale", "buy", "coupling", "take-free", "give-free", "other"),
    }
