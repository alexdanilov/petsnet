$(document).ready(function(){
    ...
    */
	...
	$('.contentBlock .tabs a').click(function(){
		if(!$(this).hasClass('active')){
			$(this).parent().find('.active').removeClass('active');
			$('.contentBlock .tabsContent').removeClass('show');
			$(this).addClass('active');
			$($(this).attr('rel')).addClass('show');
            // add section url
            $($(this).closest('.contentBlock').find('.section-link').attr('href', $(this).attr('href')));
		}
		return false;
	});
...
function bindAutoComplite(inp, url, result) {
	$(inp).autocomplete(url, {
        minChars: 1,
